...
def create_notification(user_id, title, message, type='info', espid=None):
    try:
        ...
        # Check global state for main meter flag
...
def check_device_power_range(user_id, espid, device_name, power, esp_state):
    ...
    if power > limit:
        ...
        if not esp_state["settings"].get(flag_key, False):
            create_notification(
                user_id, 
                f"ABNORMAL POWER DETECTED", 
                f"Power ({power}W) exceeded the normal range for {device_name} (Max {limit}W)!", 
                "error", 
                espid
            )
            ...
            if not esp["settings"].get('settings_loaded_from_db', False):
                try:
                    ...
                    if settings_res.data:
                        if esp["control"].get('current_limit', 50.0) == 50.0:
            ...
            if not esp.get("is_main", False):
                ...
                if budget_kwh > 0:
                    ...
                    if 50 <= percent < 75:
                        ...
                        esp["control"]["latest_command"] = 'off'
                        esp["settings"]["budget_locked"] = True
                        if not esp["settings"].get('flag_budget_100', False):
                            create_notification(user_id, f"ESP {espid} - BUDGET EXCEEDED", "100% budget reached. Device locked.", "error", espid)
