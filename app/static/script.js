...
function initDashboard() {
    ...
    initMasterSync();
    setTimeout(window.syncNILMUI, 1000);
    setInterval(window.syncNILMUI, 15000);
...
window.fetchSentraData = async function () {
    ...
    if (!start || !end) return alert("Please select both start and end dates first!");

    const originalText = btn.innerHTML;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Processing Architecture...';
    btn.disabled = true;

    try {
        const response = await fetch(`/api/analytics_data?start=${start}&end=${end}`);
        const data = await response.json();
        ...
        // 1. Update UI Stats
        const formatTime = (ts) => new Date(ts.replace(' ', 'T')).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

        safeTxt('hist-peak-load', data.stats.max_power.toFixed(1) + ' W');
        safeTxt('hist-peak-time', formatTime(data.stats.peak_time));
        safeTxt('hist-present-load', data.stats.present_power.toFixed(1) + ' W');
        safeTxt('hist-present-time', formatTime(data.stats.present_time));
        safeTxt('hist-occupancy-val', data.stats.occupancy + '%');
        document.getElementById('occupancy-circle').style.background = `conic-gradient(#6366f1 ${data.stats.occupancy}%, rgba(255,255,255,0.1) ${data.stats.occupancy}%)`;

        let carbonTons = data.stats.total_carbon_kg / 1000;
        if (carbonTons >= 1) safeTxt('hist-carbon-val', carbonTons.toFixed(2) + ' Tons');
        else safeTxt('hist-carbon-val', data.stats.total_carbon_kg.toFixed(2) + ' Kg');

        // 2. Feed Chart.js natively
        const charts = data.charts;

        // Line Chart
        const lineCtx = document.getElementById('historicalChart').getContext('2d');
        if (myHistoricalChart) myHistoricalChart.destroy();
        let gradBlue = lineCtx.createLinearGradient(0, 0, 0, 400);
        gradBlue.addColorStop(0, 'rgba(0, 243, 255, 0.6)'); gradBlue.addColorStop(1, 'rgba(0, 243, 255, 0.0)');

        myHistoricalChart = new Chart(lineCtx, {
            type: 'line',
            data: { labels: charts.timeseries.labels.map(formatTime), datasets: [{ label: 'Power (W)', data: charts.timeseries.data, borderColor: '#00f3ff', backgroundColor: gradBlue, fill: true, tension: 0.4, pointRadius: 0 }] },
            options: { responsive: true, maintainAspectRatio: false }
        });

        // Donut Chart
        if (myDonutChart) myDonutChart.destroy();
        myDonutChart = new Chart(document.getElementById('devicesDonutChart').getContext('2d'), {
            type: 'doughnut',
            data: { labels: charts.donut.labels, datasets: [{ data: charts.donut.data, backgroundColor: ['#00f3ff', '#bb13fe', '#ff0055', '#4F46E5'], borderWidth: 3, borderColor: '#0f172a' }] },
        ...
        myCarbonComparison = new Chart(document.getElementById('carbonComparisonChart').getContext('2d'), {
            type: 'bar',
            data: {
                labels: charts.carbon.labels, datasets: [
                    { label: 'Current Carbon (Kg)', data: charts.carbon.current, backgroundColor: '#8b5cf6', borderRadius: 4 },
                    { label: 'Optimized Carbon (Kg)', data: charts.carbon.optimized, backgroundColor: '#10b981', borderRadius: 4 }
                ]
            },
        ...
        myAICostChart = new Chart(document.getElementById('aiCostComparisonChart').getContext('2d'), {
            type: 'bar',
            data: {
                labels: charts.cost.labels, datasets: [
                    { label: 'Current Cost (EGP)', data: charts.cost.current, backgroundColor: '#8b5cf6', borderRadius: 4 },
                    { label: 'Optimized Cost (EGP)', data: charts.cost.optimized, backgroundColor: '#0ea5e9', borderRadius: 4 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false }
        });

        // 3. Render AI Tips and Attach Logic
        const tipsContainer = document.getElementById('ai-tips-container');
        tipsContainer.innerHTML = '';

        data.ai_tips.forEach((tip, idx) => {
            let row = document.createElement('div');
            row.style.cssText = 'display: flex; gap: 12px; padding: 12px; margin-bottom: 8px; border-radius: 8px; background: rgba(14, 165, 233, 0.12); border-left: 4px solid #0ea5e9;';
            row.innerHTML = `
                <input type="checkbox" id="ai-chk-${idx}" checked style="width:18px; height:18px; cursor:pointer;">
                <label for="ai-chk-${idx}" style="cursor:pointer; color:#e2e8f0;"><strong style="color:#38bdf8;">${tip.device} (-${tip.savings_percent}%):</strong> ${tip.tip}</label>
            `;
            tipsContainer.appendChild(row);

            // Toggle optimized values in charts when checkbox is clicked
            document.getElementById(`ai-chk-${idx}`).addEventListener('change', function () {
                myAICostChart.data.datasets[1].data[idx] = this.checked ? tip.opt_cost : tip.current_cost;
                myCarbonComparison.data.datasets[1].data[idx] = this.checked ? tip.opt_carbon : tip.current_carbon;
                myAICostChart.update();
                myCarbonComparison.update();
            });
        });

    } catch (error) {
        console.error("Backend Architecture Error:", error);
        alert("System error during data retrieval.");
    } finally {
...
// 13. NILM AI ENGINE INTEGRATION & FIXES
// ==================================================

// Fix for Supabase library not loading (TypeError: Cannot read properties of null)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
window.syncNILMUI = async function () {
    // Attempt to safely get user ID, fallback to empty string (Backend will use session)
    let uidParam = "";
    try {
        if (typeof window.currentUserId !== 'undefined' && window.currentUserId) {
            uidParam = `&user_id=${window.currentUserId}`;
        }
    } catch (e) { }

    const aiDevices = ['Fridge', 'Kettle', 'Washing Machine'];
    const deviceIdMap = {
        'Fridge': 'fridge',
        'Kettle': 'kettle',
    ...
    for (let i = 0; i < aiDevices.length; i++) {
        const deviceName = aiDevices[i];
        const devDomId = deviceIdMap[deviceName];

        try {
            const res = await fetch(`/api/nilm/plug-status?device_name=${deviceName}${uidParam}`);
            if (!res.ok) continue;

            const data = await res.json();
            if (data.error) continue;

            const powerSpan = document.getElementById(`${devDomId}-power`);
            const toggleBtn = document.getElementById(`${devDomId}-toggle`);

            if (!powerSpan) continue;

            let badgeHtml = '';

            if (data.phase === 'NO_PLUG') {
                badgeHtml = `<span style="font-size:0.75rem; color:#94a3b8;">Awaiting Smart Plug</span>`;
            } else if (data.phase === 'TRAINING_COLLECTION') {
                badgeHtml = `<span style="font-size:0.75rem; color:#facc15;"><i class="fa-solid fa-spinner fa-spin"></i> Collecting (${data.days_left}d left)</span>`;
            } else if (data.phase === 'READY_FOR_TRAINING' || data.phase === 'EVALUATION') {
                badgeHtml = `<span style="font-size:0.75rem; color:#bb13fe;"><i class="fa-solid fa-brain fa-pulse"></i> AI Training...</span>`;
            } else if (data.phase === 'COMPLETED') {
                badgeHtml = `<span style="font-size:0.75rem; color:#4ade80;"><i class="fa-solid fa-check"></i> AI Active</span>`;
                if (toggleBtn) toggleBtn.disabled = false;
            }

            if (data.phase !== 'COMPLETED') {
                powerSpan.innerHTML = badgeHtml;
            } else if (powerSpan.innerHTML.includes('AI Training') || powerSpan.innerHTML.includes('Collecting') || powerSpan.innerHTML.includes('Awaiting')) {
                powerSpan.innerHTML = badgeHtml;
            }

        } catch (e) {
            console.error(`NILM UI Sync Error for ${deviceName}:`, e);
        }
    }
};

window.forceNILMTraining = async function () {
    try {
        console.log("Triggering NILM background engine...");
        const res = await fetch('/api/nilm/trigger-engine', { method: 'POST' });
        const data = await res.json();
        console.log("[NILM Engine Response]", data);
        alert("AI Engine Status: " + (data.message || data.error));
    } catch (e) {
        console.error("Failed to trigger NILM engine:", e);
    }
};
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
function updateAIDevicesStatus() {
    fetch(`/api/nilm/dashboard-status`)
        .then(response => response.json())
        .then(data => {
            if (data.error) {
                console.log("AI Engine Status:", data.error);
                return;
            }

            const devices = ['fridge', 'kettle', 'washing_machine'];
            ...
            devices.forEach(device => {
                if (data[device]) {
                    ...
                    if (powerElement) {
                        powerElement.innerText = `${data[device].power}W`;
                    }

                    if (toggleElement) {
                        toggleElement.checked = data[device].status;
                    }
                }
            });
        })
...
setInterval(updateAIDevicesStatus, 8000);
updateAIDevicesStatus()