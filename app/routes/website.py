...
import time
from app.routes.api import gemini_model
import json
...
@website_bp.route('/api/analytics_data', methods=['GET'])
@login_required
def get_analytics_data():
    ...
    try:
        ...
        peak_time = ""
        active_readings = 0
        min_power = min([float(x.get('power', 0)) for x in data]) if data else 0
        active_threshold = min_power + 100
        ...
        for i in range(0, len(data), step):
            ...
            max_p = max([float(item.get('power', 0)) for item in window])
            ts_labels.append(window[0]['timestamp'])
            ts_power.append(max_p)
        ...
        device_energy_kwh = {}
        device_carbon_kg = {}
        device_historical_cost = {}
        last_energy_val = {}
        
        total_main_energy = 0.0
        total_main_cost = 0.0
        total_main_carbon = 0.0

        for item in data:
            p = float(item.get('power', 0))
            
            if p > max_power:
                max_power = p
                peak_time = item['timestamp']
            
            if p > active_threshold:
                active_readings += 1

            device = item.get('device_name', 'Unknown')
            current_kwh = float(item.get('energy_consumption', 0))
            
            delta = 0
            if device in last_energy_val:
                delta = current_kwh - last_energy_val[device]
                if delta < 0:
                    delta = 0 
            last_energy_val[device] = current_kwh
            
            historical_tier = str(item.get('tariff_tier', '1'))
            rate_at_timestamp = tier_prices.get(historical_tier, 0.68)
            cost_delta = delta * rate_at_timestamp
            ...
            if 18 <= hour < 23:
                c_factor = 0.55 
            elif hour >= 23 or hour < 9:
                c_factor = 0.35 
            carbon_delta = delta * c_factor

            if device.upper() == 'TOTAL (MAIN)':
                total_main_energy += delta
                total_main_cost += cost_delta
                total_main_carbon += carbon_delta
            else:
                device_energy_kwh[device] = device_energy_kwh.get(device, 0) + delta
                device_historical_cost[device] = device_historical_cost.get(device, 0.0) + cost_delta
                device_carbon_kg[device] = device_carbon_kg.get(device, 0) + carbon_delta

        # ========================================================
        # ========================================================
        sum_known_energy = sum(device_energy_kwh.values())
        aggregate_energy = total_main_energy - sum_known_energy

        if aggregate_energy > 0.05:
            device_energy_kwh['Aggregate'] = aggregate_energy
            device_historical_cost['Aggregate'] = max(0, total_main_cost - sum(device_historical_cost.values()))
            device_carbon_kg['Aggregate'] = max(0, total_main_carbon - sum(device_carbon_kg.values()))

        present_power = float(data[-1].get('power', 0))
        present_time = data[-1]['timestamp']
        occupancy = round((active_readings / len(data)) * 100) if data else 0
        total_carbon = sum(device_carbon_kg.values())

        devices_list = list(device_energy_kwh.keys())
        ai_recommendations = []
        
        if device_energy_kwh:
            exact_names = ", ".join([f'"{d}"' for d in devices_list])
            summary = "".join([f"- Device: '{d}', Energy: {round(e, 3)} kWh\n" for d, e in device_energy_kwh.items()])
            
            prompt = (
                f"Analyze this energy consumption data:\n{summary}\n"
                f"Return ONLY a JSON array. Each object MUST have exactly these keys:\n"
                f"1. 'device_name' (MUST exactly match one of: {exact_names})\n"
                f"2. 'recommendation' (short actionable tip)\n"
                f"3. 'savings_percent' (realistic integer between 5 and 25)\n"
                f"Ensure every device from the list has an entry."
            )
            try:
                import google.generativeai as genai
                
                full_prompt = prompt + "\nCRITICAL: Do NOT wrap the JSON in markdown blocks like ```json."
                ...
                try:
                    ...
                    logger.warning(f"First model failed: {inner_e}")
                ...
                if match:
                    ...
                    ai_recommendations = json.loads(clean_json)
                else:
                ...
                import logging
                logger = logging.getLogger("SENTRA_Core")
                logger.error(f"AI Analytics Parsing Error: {e}")
                
                ai_recommendations = [{"device_name": d, "recommendation": "Optimize usage patterns during peak hours to maintain efficiency.", "savings_percent": 12} for d in devices_list]
        ...
        for d in devices_list:
            cost = device_historical_cost.get(d, 0.0)
            carbon = device_carbon_kg[d]
            
            ai_data = next((x for x in ai_recommendations if x.get('device_name', '').upper() == d.upper()), None)
            savings = int(ai_data.get('savings_percent', 0)) if ai_data else 0
            tip_text = ai_data.get('recommendation', 'Monitor operations to maintain efficiency.') if ai_data else 'Monitor operations.'
            
            multiplier = 1 - (savings / 100.0)
            
            current_costs.append(cost)
            optimized_costs.append(cost * multiplier)
            current_carbons.append(carbon)
            optimized_carbons.append(carbon * multiplier)
            ...
            ai_tips.append({
                ...
                "current_cost": cost,
                "opt_cost": cost * multiplier,
                "current_carbon": carbon,
                "opt_carbon": carbon * multiplier
            })
        ...
        return jsonify({
            ...
            "charts": {
                "timeseries": {"labels": ts_labels, "data": ts_power},
                "donut": {"labels": devices_list, "data": list(device_energy_kwh.values())},
                "carbon": {"labels": devices_list, "current": current_carbons, "optimized": optimized_carbons},
                "cost": {"labels": devices_list, "current": current_costs, "optimized": optimized_costs}
            },
            "ai_tips": ai_tips
        }), 200
...
def save_config():
    ...
    try:
        ...
        for setting in settings_res.data:
            ...
            if hasattr(g, 'esps') and espid in g.esps:
                g.esps[espid]["settings"]["user_segment"] = new_tariff

            if initial_egp > 0 and hasattr(g, 'esps') and espid in g.esps:
                esp_settings = g.esps[espid]["settings"]
